...
static inline int __builtin_ctz_wrapper(uint32_t x) {
...
static inline int sched_find_first_bit(uint32_t word) {
  if (word == 0)
    return -1;
  return __builtin_ctz_wrapper(word);
}
...
void _sched_enqueue(CpuRunQueue *rq, Process *p) {
  ...
  if (p->dynamic_priority >= SCHED_PRIO_LEVELS)
    p->dynamic_priority = SCHED_PRIO_LEVELS - 1;

  priority_t prio = p->dynamic_priority;
...
Process *scheduler_create_process(const char *name, void (*entry_point)(void),
                                  priority_t prio) {
  for (int i = 0; i < SCHEDULER_MAX_PROCESSES; i++) {
    (void)entry_point; // Silence unused parameter warning until context switch
                       // is implemented
  ...
  if ((rq->total_ticks % SCHED_AGING_THRESHOLD_TICKS) == 0) {
    for (int i = 0; i < SCHEDULER_MAX_PROCESSES; i++) {
      ...
      if (p->pid != 0 && p->state == PROCESS_STATE_READY &&
          p->dynamic_priority > 0) {
        /* Bump priority (Decrement value -> Improve priority) */
